  ...
  for (int j = i; j < argc; j++) {
    ...
    if ((strcmp(argv[j], "--res") == 0) && (j < argc - 1)) {
      try {
        ...
        printf("Pixellation/Resolution overridden from default value target "
               "config file.  Now: %f\n",
               this->pixellation);
      } catch (...) {
        throw std::runtime_error("Invalid Pixellation/Resolution parameter "
    ...
    if ((strcmp(argv[j], "--mag") == 0) && (j < argc - 1)) {
      try {
        ...
        printf("Magnitude overridden from default value target config file.  "
               "Now: %f\n",
               this->mag);
...
string Target::GetName() { return this->name; }

/// Returns the target ID
/// NOTE: It is assume a single target will be simulated so this value is fixed
/// to one, but kept
/// as a function for future expansion of this program.
int Target::GetTargetID(void) { return 1; }
...
void Target::InitFlux() {
  // Multiply by waveband width (in wavenumber unit = m-1 ), collecting surface,
  ...
  // The background magnitude is in magnitude/ sq arcsec and assumed given in
  // the same band as the source
  ...
  //	cout << "Flux = " << this->flux << " photons per m, per second." <<
  // endl;
...
oi_target Target::GetOITarget(void) {
  ...
  /// \bug Spectral type output here always has some junk after it.  Not sure
  /// why though.
