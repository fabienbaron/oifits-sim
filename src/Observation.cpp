...
/// Queries the Array object for the scopes found in the comma separated string
/// "telescopes"
...
vector<Baseline *> Observation::FindBaselines(vector<Station *> stations, string exclude_baselines) {
  ...
  // Now compute all of the baselines and make a hash table for each baseline
  // value
  ...
  // Now query for all of the remaining baselines and add them to the output
  // "baselines"
...
vector<Triplet *> Observation::FindTriplets(vector<Station *> stations, string exclude_baselines) {
  ...
  for (TNameHash::iterator it = tri_names.begin(); it != tri_names.end(); ++it) {
    ...
    for (unsigned int j = 0; j < excluded_baselines.size(); j++) {
      ...
      if (this->mArray->GetTriplet(tri_name)->ContainsBaseline(bl_name)) {
        // The excluded baseline is contained in this triplet, remove it from
        // the hash
        tri_names.erase(it);
        ...
        // Now that we have found a match, invalidate the loop over baselines
        // (index j)
  ...
  // Now grab the triplets from the array and put them into the triplets
  // variable.
  for (TNameHash::iterator it = tri_names.begin(); it != tri_names.end(); ++it) {
    tri_name = it->second;
...
vector<Quadruplet *> Observation::FindQuadruplets(vector<Station *> stations, string exclude_baselines) {
  ...
  // Start by first computing all of the names of all of the possible
  // quadruplets
  int num_stations = stations.size();
  int i, j, k, l;
  ...
  for (TNameHash::iterator it = quad_names.begin(); it != quad_names.end(); ++it) {
    ...
    for (unsigned int j = 0; j < excluded_baselines.size(); j++) {
      ...
      if (this->mArray->GetQuadruplet(quad_name)->ContainsBaseline(bl_name)) {
        // The excluded baseline is contained in this quadruplet, remove it from
        // the hash
        quad_names.erase(it);
  ...
  // Now grab the quadruplets from the array and put them into the quadruplets
  // variable.
  for (TNameHash::iterator it = quad_names.begin(); it != quad_names.end(); ++it) {
    quad_name = it->second;
...
int Observation::GetNumStations(void) { return this->mStations.size(); }

Station *Observation::GetStation(int sta_index) { return this->mStations[sta_index]; }

ObsType Observation::GetObsType(void) { return this->mObsType; }

vector<Observation *> Observation::ParseCommandLine(Array *array, char *argv[], int i, int argc, string comment_chars) {
  // Parsing the command line is a little funny as we permit some observations
  // to
...
vector<Observation *> Observation::ParseCommandLineObs(Array *array, char *argv[], int i, int argc) {
  ...
  for (int j = i + 1; j < argc; j++) {
    // TODO: Add in some way to jump out of this loop before we hit argc
    // otherwise
  ...
  if (have_all[0] && have_all[1] && have_all[2]) {
    ...
    throw std::runtime_error("Not all observation parameters were specified on "
                             "the command line.  You need --start, --stop, and "
                             "--every at a minimum.");
  }
}

/// Reads in an properly formatted observation file in a formats defined by
/// file_type:
///     0: A list of hour angles (in decimal hours)
///     1: A descriptive list of the observation (see
///     ReadObservation_Descriptive() for more info)
...
bool Observation::HasVIS(void) { return this->mbHasVIS; } // A simple function to see if the observation has powerspectra.

bool Observation::HasV2(void) { return this->mbHasV2; }

// A simple function to see if the observation has triplets.
bool Observation::HasT3(void) { return this->mbHasT3; }

// A simple function to see if the observation has quadruplets.
bool Observation::HasT4(void) { return this->mbHasT4; }
