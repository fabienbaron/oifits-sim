 ...
 *      Author: fbaron (via. MROI simulator, rewritten for class by
 * bkloppenborg)
 *
...
double NoiseModel_Tatulli2006::GetVis2Var(Array *array, Combiner *combiner, SpectralMode *spec_mode, Target *target, Baseline *baseline, UVPoint UV,
                                          int wavelength_num) {
  // Here we assume that the noise is composed of two primary terms.  (1)
  // variance in the coherent flux, and (2) variance in the photometric flux
  // We start computing these values by finding how many photons make it to the
  // combiner:
  ...
  // Secondly we compute the coherent flux and it's variance.  Start by looking
  // up the squared visibility:
  ...
  // Now compute the variance of the vis2.  Note, we have two telescopes whose
...
// Compute the error for the closure phase using an unbiased estimator.  This
// function includes noise terms from
// photons and the detector only (i.e. NO atmospheric noise!)
double NoiseModel_Tatulli2006::GetT3PhaseVar(Array *array, Combiner *combiner, SpectralMode *spec_mode, Target *target, Triplet *triplet, UVPoint uv1,
                                             UVPoint uv2, int wavelength_num) {
  // Here we assume that the noise may be computed from the detector and photon
  // noise terms added in quadrature.  The paper
...
double NoiseModel_Tatulli2006::GetT4PhaseVar(Array *array, Combiner *combiner, SpectralMode *spec_mode, Target *target, Quadruplet *quadruplet, UVPoint uv1,
                                             UVPoint uv2, UVPoint uv3, int wavelength_num) {
  return 5.;
}
...
double NoiseModel_Tatulli2006::CoherentFlux(Array *array, Combiner *combiner, double n_photons, double vis2) {
  return n_photons * n_photons * vis2 / pow(array->GetNumStations(), 2) + n_photons + combiner->GetNumFringePixels() * pow(combiner->read_noise, 2);
}
...
double NoiseModel_Tatulli2006::VarCoherentFlux(Array *array, Combiner *combiner, double n_photons, double vis2) {
  ...
  // The variance in coherent flux comes from three terms (see paper, equations
  // 6 and 8)
  // (1) photon noise
...
double NoiseModel_Tatulli2006::ComputeT3PhVar(int nt[7], double nph[7], double v_ij, double v_jk, double v_ik, double t3amp) {
  ...
  double temp = (nt[3] - 2 * t3amp) * nt[3] / nph[3] + (nt[2] * (v_ij + v_jk + v_ik)) * nt[2] / nph[2] -
                (v_ij * v_ij + v_jk * v_jk + v_ik * v_ik + 2 * (v_ij * v_jk + v_ij * v_ik + v_jk * v_ik)) * nt[2] / nph[2] +
                (nt[1] * (v_ij * v_jk + v_ij * v_ik + v_jk * v_ik)) * nt[1] / nph[1] - (2 * t3amp * (v_ij + v_jk + v_ik)) * nt[1] / nph[1];
...
double NoiseModel_Tatulli2006::ComputeT3DetVar(int nt[7], double nph[7], double v_ij, double v_jk, double v_ik, double t3amp, double rn[7], int npix[7]) {
  // TODO: this could be slighly improved by pre-computing nt/nph values before
  // this function.

  // Implementing equation 17.  Note, we factored out 1 / (2 t3amp^2)
  double temp = (npix[3] * rn[6] + 3 * npix[2] * rn[6]) * nt[6] / nph[6] + (v_ij + v_jk + v_ik) * (3 * npix[1] * rn[4] + npix[2] * rn[4]) * nt[4] / nph[4] +
