...
// Fabien Baron
void Interp2d(Matrix<double> &dataIn, Matrix<double> &dataOut, double offsetX, double offsetY, double scale, double rotation) {
  ...
  for (int iy = 0; iy < nyOut; iy++) {
    for (int ix = 0; ix < nxOut; ix++) {
      ...
      dataOut[ix][iy] =
          fracX * (fracY * dataIn[ix1][iy1] + (1.0 - fracY) * dataIn[ix1][iy0]) + (1.0 - fracX) * (fracY * dataIn[ix0][iy1] + (1.0 - fracY) * dataIn[ix0][iy0]);
    }
  }
}
...
void mat2fits(Matrix<Complex> &m,
              char *filename) // TBD: fix row major vs colum major
{
  int status = 0;
  ...
  if (status == 0)
    fits_create_img(fptr, DOUBLE_IMG, naxis, naxes, &status);
  if (status == 0)
    fits_write_img(fptr, TDOUBLE, fpixel, nelements, &ptrimgreal[0], &status);
  if (status == 0)
    fits_close_file(fptr, &status);
  ...
  if (status == 0)
    fits_create_img(fptr, DOUBLE_IMG, naxis, naxes, &status);
  if (status == 0)
    fits_write_img(fptr, TDOUBLE, fpixel, nelements, &ptrimgimag[0], &status);
  if (status == 0)
    fits_close_file(fptr, &status);
...
void fits2mat(const char *fname, Matrix<double> &m) {
  ...
  // The pixellation parameter has been moved to the SOURCE/Target definition
  // file.
  ...
  //            /// \exception runtime_error The Target FITS file is missing
  //            PIXEL keyword!
  //            /// This means that the input FITS file is missing the
  //            PIXEL(lation) parameter
  //            /// which should be defined somewhere in the FITS header.
  //            throw std::runtime_error("Target FITS File missing PIXEL
  //            keyword!");
  //        }
  //        else
  //        {
  ...
  if (status == 0)
    fits_read_img(fptr, TDOUBLE, fpixel, npixels, &nullval, ptrimg, &anynull, &status);
  if (status == 0)
    fits_close_file(fptr, &status);
...
void FFT(Matrix<Complex> &min, Matrix<Complex> &mout, int direction, int dccenter) {
  ...
  fftw_complex *out = (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * nfft * nfft);
  ...
  if (direction >= 0)
    ...
    p = fftw_plan_dft_2d(nfft, nfft, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
  fftw_execute(p);
  fftw_destroy_plan(p);
  fftw_free(in);
  ...
  if (dccenter == 0) {
    for (int iy = 0; iy < nfft; iy++)
      for (int ix = 0; ix < nfft; ix++)
        mout[ix][iy] = Complex(out[ix + nfft * iy][0], out[ix + nfft * iy][1]) / double(nfft);
  } else {
    for (int iy = 0; iy < nfft / 2; iy++) {
      for (int ix = 0; ix < nfft / 2; ix++) {
        mout[ix][iy] = Complex(out[nfft / 2 + ix + nfft * (iy + nfft / 2)][0], out[nfft / 2 + ix + nfft * (iy + nfft / 2)][1]) / double(nfft);
      }

      for (int ix = nfft / 2; ix < nfft; ix++) {
        mout[ix][iy] = Complex(out[ix - nfft / 2 + nfft * (iy + nfft / 2)][0], out[ix - nfft / 2 + nfft * (iy + nfft / 2)][1]) / double(nfft);
      }
    }
