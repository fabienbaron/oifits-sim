...
void SpectralMode::ImportFile(string filename, string combiner, string comment_chars) {
  ...
  int max_params = 2; // This is the number of non-telescope parameters found in
  ...
  if (lines.size() - line_number < 1)
    throw std::runtime_error("At least one wavelength + bandpass is required "
                             "for a spectral mode.  Check configuration file.");
  ...
  // Parse the text file, pull out the wavelengths and compute the associated
  // wavenumbers.
  double waven_low = 0;
  double waven_high = 0;
  ...
  // Lastly compute the median wavelength (added in original code by AS
  // 2010-06-24)
