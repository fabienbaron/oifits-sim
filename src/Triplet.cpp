...
Triplet::Triplet(Array *array, Station *station1, Station *station2, Station *station3) {
  ...
  //        mBaselines[0]->GetName().c_str(),  mBaselines[1]->GetName().c_str(),
  //        mBaselines[2]->GetName().c_str());
}

/// Returns the name of this triplet as the names of the stations separated by
/// hyphens, e.g.:
///     S1-E1-E2
string Triplet::GetName(void) { return this->name; }
...
// complex<double> Triplet::ComputeT3Error(Target & target, UVPoint uv_ab,
// UVPoint uv_bc, UVPoint uv_ac)
//{
//    /// \bug T3pectrum error is set to 0.001 by default
//    return complex<double>(0.001, 0.001);
//}

complex<double> Triplet::ComputeT3(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac, double wavelength, double dwavelength) {
  ...
  complex<double> BC = mBaselines[1]->GetVisibility(target, uv_bc, wavelength, dwavelength);
  complex<double> AC = mBaselines[2]->GetVisibility(target, uv_ac, wavelength, dwavelength); // Note, this is AC NOT CA

  // Now compute the bispectrum.  We take the conjugate of AC to form CA.
  return AB * BC * conj(AC);
}

/// Returns a boolean to indicate if the baseline specified by bl_name is
/// involved in this triplet.
bool Triplet::ContainsBaseline(string bl_name) {
  /// \todo We could probably implment this as a hash lookup instead of a string
  /// operation
...
double Triplet::GetT3Amp(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac, double wavelength, double dwavelength) {
  complex<double> bis = GetT3(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
  return abs(bis);
}

// Wrapper function for the UV-coordinate taking versions.
double Triplet::GetT3Amp(Target &target, double hour_angle, double wavelength, double dwavelength) {
  ...
  uv_ac.Scale(1. / wavelength);

  return GetT3Amp(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
}

// Returns the norm (amplitude) of the bispectrum
double Triplet::GetT3AmpErr(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac, double wavelength, double dwavelength) {
  /// \bug Returns 0.001 by default
  return 0.001;
}

// Wrapper function for the UV-coordinate taking versions.
double Triplet::GetT3AmpErr(Target &target, double hour_angle, double wavelength, double dwavelength) {
  UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
  uv_ab.Scale(1. / wavelength);
  UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
  uv_bc.Scale(1. / wavelength);
  ...
  return GetT3AmpErr(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
}

// Wrapper function for the UV-coordinate taking versions.
double Triplet::GetT3Phi(Target &target, double hour_angle, double wavelength, double dwavelength) {
  UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
  uv_ab.Scale(1. / wavelength);
  UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
  uv_bc.Scale(1. / wavelength);
  ...
  return GetT3Phi(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
}

/// Returns the argument (phase) of the bispectra
double Triplet::GetT3Phi(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac, double wavelength, double dwavelength) {
  complex<double> bis = GetT3(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
  return arg(bis);
}

// Wrapper function for the UV-coordinate taking versions.
double Triplet::GetT3PhiErr(Target &target, double hour_angle, double wavelength, double dwavelength) {
  UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
  uv_ab.Scale(1. / wavelength);
  UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
  uv_bc.Scale(1. / wavelength);
  ...
  return GetT3PhiErr(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
}

double Triplet::GetT3PhiErr(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac, double wavelength, double dwavelength) {
  /// \bug Returns 0.001 by default.
  return 0.001;
}

// Computes the bispectra from the three baselines in this triplet.
complex<double> Triplet::GetT3(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac, double wavelength, double dwavelength) {
  ...
  if (mT3Values.find(hash_key) != mT3Values.end()) {
    ...
    // The value did not exist in the hash table, we need to compute and store
    // it.
    bis = ComputeT3(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
...
complex<double> Triplet::GetT3(Target &target, double hour_angle, double wavelength, double dwavelength) {
  UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
  uv_ab.Scale(1. / wavelength);
  UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
  uv_bc.Scale(1. / wavelength);
  ...
  return GetT3(target, uv_ab, uv_bc, uv_ac, wavelength, dwavelength);
}
...
string Triplet::GetHashKey(Target &target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac) {
  /// \todo It may be necessary for the doubles coming into this function to be
  /// cast into some
  /// finite floating point format.

  /// \todo This function is in common with the Baseline class, need to factor
  /// this code.
