...
UVPoint Baseline::UVcoords(double hour_angle, double target_declination) {
  // First convert all values into radians (they should be degrees or decimal
  // hours (of time) before now.
  ...
  /// \note The coordinates (u,v) calculated here appear to be (-u, -v) with
  /// respect to CHARA+MIRC data.
  /// Given that the UV plane is symmetric, this doesn't matter.

  return uv;
}

string Baseline::GetName(void) { return this->name; }
...
string Baseline::GetHashKey(Target &target, UVPoint uv) {
  ...
  /// \todo This function is in common with the Triplet class, need to factor
  /// this code.
...
void Baseline::SetHashKey(complex<double> vis, Target &target, UVPoint uv)
{
 string hash_key = GetHashKey(target, uv);
 mVisValues[hash_key] = vis;
}
  ...
  } else // A resolved object
  {
    ...
    for (int ii = 0; ii < nx; ii++)
      for (int jj = 0; jj < ny; jj++) {
        ...
        //* sinc( dwavelength/wavelength * (uv.u * (double)(ii - nx / 2 ) - uv.v
        //* (double)(jj - ny / 2)) * target.pixellation * milliarcsec);
      }
