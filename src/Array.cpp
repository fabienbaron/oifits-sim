...
void Array::ImportFile(string filename, string comment_chars) {
  ...
  int max_params = 8; // This is the number of non-telescope parameters found in
  ...
  // which I think is a nice feature.  To do this, we first need to see if a
  // file with the
  // name, filename, really exists.
  // TODO: Use a global variable, read in from a configuration file, to specify
  // "../etc" below.
  ...
  for (line_number = 0; line_number < max_params; line_number++) {
    ...
    if (results[0] == "coord") {
      try {
        ...
        throw std::runtime_error("Invalid Coordinate definition specification "
                                 "for telescope positions.");
      }
    }
  ...
  // Do some quick error checking on the file format.  First, make sure we've
  // got all of the parameters:
  ...
  if (lines.size() - line_number < 2)
    throw std::runtime_error("At least two telescopes are required for an "
                             "array!  Check configuration file.");

  // Now iterate through the telescope definitions.  There are four entries
  // above
...
// This function permits a few target parameters to be overridden via. command
// line arguments
  ...
  for (int j = i; j < argc; j++) {
    ...
    if ((strcmp(argv[j], "--r0") == 0) && (j < argc - 1)) {
      try {
        ...
        printf("r0 overridden from default value in array config file.  Now: "
               "%f m\n",
               this->r0);
      } catch (...) {
    ...
    if ((strcmp(argv[j], "--wind_speed") == 0) && (j < argc - 1)) {
      try {
        ...
        printf("wind_speed overridden from default value in array config file. "
               " Now: %f km/s\n",
               this->wind_speed);
      } catch (...) {
...
Station *Array::GetStation(int station_index) { return &this->stations[station_index]; }

Station *Array::GetStation(string sta_name) { return this->sta_hash[sta_name]; }

double Array::GetLatitude(void) { return this->latitude; }

double Array::GetLongitude(void) { return this->longitude; }

double Array::GetAltitude(void) { return this->altitude; }

int Array::GetNumStations(void) { return this->stations.size(); }

string Array::GetArrayName(void) { return this->arrayname; }
...
Triplet *Array::GetTriplet(string triplet_name) { return this->tri_hash[triplet_name]; }
...
Quadruplet *Array::GetQuadruplet(string quadruplet_name) { return this->quad_hash[quadruplet_name]; }
...
oi_array Array::GetOIArray(void) {
  /// \todo This could be claned up to use datamembers instead of function
  /// calls.
...
double Array::Get_r0(void) { return this->r0; }

double Array::GetWindSpeed(void) { return this->wind_speed; }

double Array::GetThroughput(void) { return this->throughput; }
