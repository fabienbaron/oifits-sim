...
// Computes the number of photons sent through the system per coherent
// integration period
// including throughput losses of the array only (combiner losses should be
// calculated elsewhere).
// Note: this function assumes all telescopes are of the same diameter and have
// the same gain
// also, this function doesn't split the photons between photometric and data
// channels, that should be done
// elsewhere.
double NoiseModel::NumPhotons(Array *array, Target *target, Combiner *combiner, SpectralMode *spec_mode, int wavelength_num) {
  ...
  // re-computing it each time.  These are all fast operations, at worst
  // O(n_telescopes)
  // == linear.
  ...
  // TODO: right now we assume all stations are the same diameter. We will need
  // to
