 ...
 *
 * Note: This differs from the MROI simulator in that it uses wavenumbers
 * instead of
 * wavelengths.
 */
...
public:
  ...
  double GetT3PhaseVar(Array *array, Combiner *combiner, SpectralMode *spec_mode, Target *target, Triplet *triplet, UVPoint uv1, UVPoint uv2,
                       int wavelength_num);

  double GetT4PhaseVar(Array *array, Combiner *combiner, SpectralMode *spec_mode, Target *target, Quadruplet *quadruplet, UVPoint uv1, UVPoint uv2, UVPoint uv3,
                       int wavelength_num);
